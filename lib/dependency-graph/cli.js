...
/**
 * Prompts:
 * - whether to interrogate nodes for Frost Foundation or All
 * - which dependencies to interrogate
 *
 * @type {Object}
 */
const promptSchema = {
  properties: {
    node: {
      name: 'allfrost',
      message: 'Do you wish to interrogate the nodes for just the Frost Foundation ("frost") or All ("all") dependencies?',
      validator: /^frost$|^all$/,
    ...
    dependencies: {
      name: 'dependencies',
      message: 'Do you wish to interrogate the nodes for dependencies ("dependencies"), devDependencies ("devDependencies"), or both ("both")?',
      validator: /^dependencies$|^devDependencies$|^both$/,
      warning: 'Must respond "dependencies", "devDependencies", or "both"',
...
prompt.get(promptSchema, function (err, result) {
  const graph = new DependencyGraph(result.allfrost === 'all' ? true : false, result.dependencies)
