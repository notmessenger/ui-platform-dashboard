    ...
    .then(responses => {
        responses.forEach(response => {
            try {
                ...
                for (let devDependency in file.devDependencies) {
                    if (repositoriesPublishedByCiena.map(repository => repository.name).includes(devDependency)) {
                        info.devDependencies.push(devDependency)
                    }
                }
