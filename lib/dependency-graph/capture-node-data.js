...
repositories.forEach((repository) => {
    repository.repositories.forEach((entry) => {
        ...
        repositoriesPublishedByCiena.push(entry.name)
    })
})
...
Promise
    ...
    .then(responses => {
        responses.forEach(response => {
            try {
                ...
                for (let dependency in file.dependencies) {
                    if (repositoriesPublishedByCiena.includes(dependency)) {
        ...
        fs.writeFileSync(`${dataDirectory}/nodes.json`, JSON.stringify(foundationRepositoriesGraphData, null, '  '))
    })
    .catch(err => {
        console.log(err)
    })
