...
module.exports = class DependencyGraph {
   ...
   *
   * @param {Boolean} [true] allNodes Whether all nodes or just Frost Foundation nodes are included in graph
   * @param {String} ["dependencies"] whichDependencies Whether to generate graphs for "dependencies", "devDependencies", or "both"
   * @returns {undefined}
   */
  constructor (allNodes = true, whichDependencies = 'dependencies') {
    const DepGraph = require('dependency-graph').DepGraph
    const fs = require('fs')

    this.dependencyGraph = new DepGraph({circular: whichDependencies ? true : false})
    this.dependencyType = whichDependencies
   ...
   */
  _addDependencies () {
    if (this.dependencyType === 'both') {
      this.nodes.forEach((node) => {
        node.dependencies.forEach((dependency) => {
          this.dependencyGraph.addDependency(node.name, dependency)
        })

        node.devDependencies.forEach((dependency) => {
          this.dependencyGraph.addDependency(node.name, dependency)
        })
      })
    } else {
      this.nodes.forEach((node) => {
        node[this.dependencyType].forEach((dependency) => {
          this.dependencyGraph.addDependency(node.name, dependency)
        })
      })
    }
